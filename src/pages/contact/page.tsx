    ...
    <div className="min-h-screen bg-background" dir={isRtl ? "rtl" : "ltr"}>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
          <motion.div initial={{ opacity: 0, x: isRtl ? 28 : -28 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}><h2 className="text-4xl font-light mb-6 text-foreground" style={{ fontFamily: "Cormorant Garamond, serif" }}>{t("contact.subtitle")}</h2><p className="text-muted-foreground mb-10 leading-relaxed" style={{ fontFamily: "Montserrat, sans-serif", fontWeight: 300, fontSize: "0.88rem" }}>{t("contact.desc")}</p><div className="space-y-6">{[{ icon: MapPin, label: t("contact.address"), value: "Alger" }, { icon: Phone, label: t("contact.phone"), value: "+213 561 45 82 86" }, { icon: Mail, label: t("contact.email"), value: "Contact@atlas-miel.com" }].map((item) => <div key={item.label} className="flex items-center gap-4"><div className="p-3 border border-border"><item.icon size={18} className="text-primary" /></div><div><p className="text-xs tracking-widest uppercase text-muted-foreground" style={{ fontFamily: "Montserrat, sans-serif", fontSize: "0.6rem" }}>{item.label}</p><p className="text-foreground" style={{ fontFamily: "Montserrat, sans-serif", fontSize: "0.88rem" }}>{item.value}</p></div></div>)}</div></motion.div>
          <motion.form initial={{ opacity: 0, x: isRtl ? -28 : 28 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} onSubmit={handleSubmit(onSubmit)} className="space-y-5">
